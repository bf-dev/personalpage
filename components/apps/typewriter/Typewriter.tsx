...
export default function Typing() {
	...
	const calculateMetrics = useCallback(() => {
		...
		const calculatedAccuracy =
			currentIndex > 0 ? Math.round(((currentIndex - errorCount) / currentIndex) * 100) : 100;
			...
			<div className="font-mono py-3 px-3">
				...
				{isPaused && !isCompleted && (
					<>
						...
						<span className="text-white/70 text-sm">PAUSED</span>
					</>
				)}
				{isCompleted && (
					<>
				...
				{!startTime && !isCompleted && !isPaused && (
					<>
						...
						<span className="text-blue-400/70 text-sm">READY</span>
					</>
				)}
			</div>
