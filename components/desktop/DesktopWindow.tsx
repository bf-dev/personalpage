	...
	return (
		<motion.div
			...
			style={{
				...
				transition:
					'width 0.3s ease-in-out, height 0.3s ease-in-out, border-radius 0.2s ease-in-out',
				...style,
			}}
		>
			{/* Window header/handle bar */}
			<div className="flex items-center p-2 cursor-grab" onPointerDown={handlePointerDown}>
				<div className="flex-1 flex items-center justify-center flex-col">
					<div
						className={`h-1 transition-all mt-1 bg-white/20 rounded-full ${preClose ? 'w-4' : preMaximize ? 'w-16' : 'w-8'} `}
					/>
					<AnimatePresence>
						{preClose && (
							<motion.p
